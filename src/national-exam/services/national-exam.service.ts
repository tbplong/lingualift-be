...
import { NationalExam, NationalExamDocument, NationalExamStatus } from '../schemas';
...
export class NationalExamService {
  ...
  */
  public async getNationalExamsV2(
      ...
      $or: [
        {
          $and: [
            search
              ...
              : {},
            { isDeleted: false },
            levelRange ? { level: { $gte: levelRange[0], $lte: levelRange[1] } } : {},
          ],
        },
    ...
    const pinnedExams = examFiles.filter((file) => file.status === NationalExamStatus.HOT);
    const unpinnedExams = examFiles.filter((file) => file.status !== NationalExamStatus.HOT);
  ...
  public async updateNationalExam(
    ...
    const { title, status, level, questions, videoLink } = updateNationalExamRequestInterface;
