...
export class MinioStorageService {
  ...
  public async getPresignedUploadUrl(bucket: string, key: string, expires = 300): Promise<string> {
    try {
  ...
  public async getDownloadStream(bucket: string, key: string): Promise<Readable> {
    try {
      await this.ensureBucketExist(bucket);
  ...
  public async getLinkToDownload(bucket: string, key: string, expires = 300): Promise<string> {
    await this.ensureBucketExist(bucket);
  ...
  public async putObject(file: Express.Multer.File, bucket: string, key: string): Promise<void> {
    try {
      await this.minioClient.putObject(bucket, key, file.buffer, file.size);
    } catch (error: any) {
      throw new InternalServerErrorException(`Failed to put object: ${error.message}`);
    }
  }
  ...
  private async ensureBucketExist(bucket: string): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException(`Bucket ${bucket} doesn't exists!`);
    }
  }
}
